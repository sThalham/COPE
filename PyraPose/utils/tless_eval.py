...
def evaluate_tless(generator, model, data_path, threshold=0.5):
    ...
    recall = np.zeros((31), dtype=np.float32)
    precision = np.zeros((31), dtype=np.float32)
    detections = np.zeros((31), dtype=np.float32)
    det_precision = np.zeros((31), dtype=np.float32)
    ...
    recall_all = np.sum(recall) / 30.0
    precision_all = np.sum(precision) / 30.0
    detections_all = np.sum(detections) / 30.0
    det_precision_all = np.sum(det_precision) / 30.0
    print('ALL: ')
