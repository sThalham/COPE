    ...
    for set in syns:
        ...
        for samp in os.listdir(rgbPath):
            ...
            for i in range(len(gtImg)):
                ...
                for j in range(np.random.randint(100)):
                    t = np.array([(np.random.random() * 3.0) - 1.5, (np.random.random() * 2) - 1.0, 0.5 + (np.random.random() * 0.75)]) * 1000.0
                    R = tf3d.euler.euler2mat(np.random.random() * 2 * np.pi, np.random.random() * 2 * np.pi, np.random.random() * 2 * np.pi)
                    ...
                    ren.set_light(light_pose, light_color, light_ambient_weight, light_diffuse_weight, light_spec_weight,
                                  light_spec_shine)
                    ren.render_object(obj_id, R_list, t_list, fxca, fyca, cxca, cyca)

                    ren_img = ren.get_color_image(obj_id)
                    rgbImg = np.where(ren_img > 0, ren_img, rgbImg)
